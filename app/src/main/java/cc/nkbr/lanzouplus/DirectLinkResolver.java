...
final class DirectLinkResolver implements AutoCloseable {
  ...
  private final class Request {
        ...
        Request(String url,boolean confirmed,long sequence,String password){this.url=url;this.routeGroup=LanzouCore.directRouteGroupKey(url);this.confirmed=confirmed;this.sequence=sequence;this.password=password==null?"":password;}
                void resolveNow(){try{LanzouCore.DirectLink link=core.resolveDirectBalanced(url,password,sequence);if(link==null||link.url==null||link.url.isEmpty())throw new IllegalStateException("未解析到下载直链");long at=clock.now();if(!password.isEmpty())rememberPassword(url,password);recordPressureFreeSuccess();finished(this,link.url,at,null);}catch(LanzouCore.DirectPasswordException rejected){boolean hadPassword=!password.isEmpty();if(hadPassword)forgetPassword(url);awaitPassword(this,hadPassword);}catch(InterruptedException interrupted){Thread.currentThread().interrupt();if(!closed)finished(this,null,0,failureMessage(interrupted));}catch(Exception error){++failures;if(upstreamPressure(error)&&adaptToUpstreamPressure()){synchronized(lock){if(!done&&!closed)resumePending=true;}return;}long delay=LanzouCore.directRetryDelay(error,failures);if(delay>0)defer(this,delay);else finished(this,null,0,failureMessage(error));}}
  }
}
