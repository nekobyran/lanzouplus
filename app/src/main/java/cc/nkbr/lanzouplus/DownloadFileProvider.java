...
public final class DownloadFileProvider extends ContentProvider {
  ...
  private File resolve(Uri uri)throws FileNotFoundException{try{java.util.List<String> segments=uri.getPathSegments();if(segments.isEmpty()||segments.size()>2)throw new FileNotFoundException();boolean verified=segments.size()==2&&"verified".equals(segments.get(0));if(segments.size()==2&&!verified)throw new FileNotFoundException();String name=segments.get(segments.size()-1);if(name.isEmpty())throw new FileNotFoundException();Context context=getContext();if(context==null)throw new FileNotFoundException();File root=(verified?new File(context.getFilesDir(),"verified-updates"):android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS)).getCanonicalFile(),file=new File(root,name).getCanonicalFile();if(!root.equals(file.getParentFile())||!file.isFile())throw new FileNotFoundException();return file;}catch(IOException error){throw new FileNotFoundException(error.getMessage());}}
